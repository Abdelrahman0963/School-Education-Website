    ...
    return (
        <section className='!p-4 md:!py-6 md:!px-16 w-full h-full md:min-h-[calc(100vh-10rem)] overflow-hidden'>
            <div className="flex flex-col items-center gap-4">
                <div className='bg-white border-2 !p-2 rounded-md'><p className='text-[#333333] text-sm' style={{ fontFamily: 'var(--font-raleway)' }}>Mission & Visions</p></div>
            ...
            <div className="!mt-10 flex flex-col md:flex-row gap-4 md:gap-8 justify-center items-center">
                {visionsData.map((Visions, index) => (
                    <div key={index} className="md:!px-10 md:!py-8 !px-6 !py-4 shadow-[7px_8px_6px_#000] border-2 border-black bg-white rounded-md w-full md:h-72 h-60 flex flex-col overflow-hidden">
                        ...
                        <p className='!mt-8 text-[#333333] text-start md:text-[16px] text-sm'>{Visions.description}</p>
                    </div>
                ))}
            </div>
        </section>
